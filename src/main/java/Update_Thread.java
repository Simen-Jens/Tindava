    ...
    public void run(){
        ...
        while (runn){
            ...
            try {
                ...
                try{
                    cmd.cmd_messageDiscord((":skull_crossbones: well shit, <@109348691525058560> sucks at programming :skull_crossbones:\nupdate thread in danger, error message sent to server owner(" + cmd.client.getGuilds().get(0).getOwner().mention() + ")"),cmd.client.getGuilds().get(0).getChannelByID(cmd.defaultChannels.split(" ")[0]), false, false);
